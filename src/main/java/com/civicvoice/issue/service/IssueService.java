...
public class IssueService {
    ...
    @Transactional
    public IssueResponse updateIssue(UUID id, IssueRequest.Update request) {
        Issue issue = findOrThrow(id);
        User actor = currentUser();

        if (issue.getReporter() == null || !issue.getReporter().getId().equals(actor.getId())) {
            throw new BusinessRuleException("Only the original reporter can edit this issue.");
        }
        ...
        log.info("Issue updated: {} by {}", issue.getId(), actor.getEmail());

        auditService.log("ISSUE_UPDATED", "ISSUE", issue.getId(), null, issue, actor);
    ...
    public IssueResponse updateStatus(UUID id, IssueRequest.UpdateStatus request) {
        ...
        notificationService.notifyStatusChange(issue.getId(), oldStatus, request.newStatus(), actor.getId());
    ...
    public IssueResponse assignIssue(UUID id, IssueRequest.Assign request) {
        ...
        issueRepository.save(issue);
        notificationService.notifyAssignment(issue.getId(), authority.getId());
    ...
    public Map<String, Object> toggleUpvote(UUID id) {
        ...
        if (upvoteRepository.existsByIssueIdAndUserId(id, user.getId())) {
            ...
            notificationService.notifyUpvote(issue.getId(), user.getId());
    ...
    public IssueResponse.CommentResponse addComment(UUID issueId, IssueRequest.AddComment request) {
        ...
        notificationService.notifyComment(issue.getId(), author.getId(), request.content());
